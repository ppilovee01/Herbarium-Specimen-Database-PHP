...
4. **การเข้าใช้งานและบัญชีทดสอบ (Default Test Accounts)**
   - เข้าใช้งานทางเว็บบราวเซอร์ผ่านที่อยู่: `http://localhost/DataWeb`
   - **บัญชีสำหรับใช้ทดสอบระบบ (ที่บันทึกอยู่ในฐานข้อมูล):**
     * **บัญชีสิทธิ์ผู้ดูแลระบบ (Admin):** Username `test1` หรือ `test3`
     * **บัญชีสิทธิ์ผู้ใช้งานทั่วไป (User):** Username `test2`
   - *หมายเหตุ:* หากคุณลืมรหัสผ่านของบัญชีทดสอบเดิม สามารถทำการสมัครสมาชิกใหม่ได้ที่หน้าเว็บไซต์ผ่านปุ่ม **เข้าสู่ระบบ -> สมัครสมาชิกใหม่** จากนั้นเข้าแก้ไขฟิลด์ `role` ในตาราง `users` จาก `user` เป็น `admin` ผ่าน phpMyAdmin เพื่ออัปเกรดสิทธิ์หลังบ้านได้ทันที
